...
1. **The game**: `game/` — a Monogatari-engine VN that must ship as a
   ...
   > codex and the pure-CSS mecha skin (`vendor/mecha-ui.css`). Read
...
- Game code (story script, HUD, codex, route atlas, boot) lives in
  ...
  change behavior.
- UI skin: `game/vendor/mecha-ui.css`, loaded **last** so it wins the cascade
  over `custom-ui.css`. It is pure CSS (no JS driver): button bevels, chamfers
  and 2.5D tilt are drawn with gradients, borders, `clip-path` and CSS 3D
  transforms; the HUD instrument rail, telemetry ticker and title block are
  static markup in `index.html`. The alert state is wired from game.js, which
  sets `html.mech-caution` / `html.mech-alarm` from `akatomi_alert`.
