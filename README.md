...
- `design/` — UI art direction (`concepts/`), reference screenshots
  (`preview/shots/`, JPEG only), the dev-only **three-ui prototype**
  (`preview/three-ui-demo.html`: AI plates + three.js 3D parts + GLSL over a
  CSS skeleton, see [`THREE_UI.md`](design/THREE_UI.md)) and the mecha preview
  harness.
