...
`border-image: … 14% fill` so the machined bevel keeps its thickness at any
button size — the reason a key looks machined at 26px and at 90px. On `file://` the black surround is trimmed by
