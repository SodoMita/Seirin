...
| **`border-image` slice regions start at the image edge** | keys wore a black foam frame: the slice included the plate's black screen-blend surround | crop the surround at ingest (`convert -crop 631x648+382+50`), then slice inside the bevel: `9→14% fill` |
