...
characters land on attempt one, Splash historically took eleven. This file is
...
ends. The repo's own history shows the cost: `characters/` contains
`splash_water_character_reference_v2` … `v11`, `krea-2-turbo_a_Мия_Кагэцуки…`,
and `flux-2-dev_a_Fix_AI_artifacts__no.jpeg` — nobody can now tell what was
asked for, what changed between versions, or why v11 was the keeper. That
knowledge was thrown away, and the next agent has to rediscover it by burning
generations.
...
Verdict: REJECT
Kept: front and 3/4 views are on-model; palette correct.
Broke: side view lost the visor entirely; back view added a second tail.
Change for next: state the visor in the staging section as well as identity,
and add "the same single asymmetric tail in all four views" explicitly.
```
...
If refinement keeps failing, the fault may be the design, not the prompt.
Symptoms: the silhouette doesn't read even when the generator obeys perfectly;
two characters keep coming out similar; the memory point never survives.
