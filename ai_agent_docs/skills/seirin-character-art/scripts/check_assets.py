...
def main() -> int:
    ...
    # The core expression set is fixed; character-specific extras come from
    # approved design answers, which may not exist yet.
    core = ["neutral", "smile", "grin", "angry", "sad", "surprise", "blush",
            "serious"]
    ...
    for cid, char in chars.items():
        ...
        if not have:
            ...
            continue
        extras = char.get("answers", {}).get("expression") or ""
        want = set(core) | {w for w in re.findall(r"`([a-z0-9-]+)`", str(extras))}
        ...
        if extra:
            warnings.append(f"[{cid}] expressions not in the approved answers: "
                            f"{', '.join(extra)}")
