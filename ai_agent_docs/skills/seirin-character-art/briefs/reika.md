# Reika «Ironheart» Takashiro — art brief

`id: reika` · iron_requiem · SSR · density 9/10 · appeal track **hero_glamour**

> Blamed for a crash she did not cause, and now she brings the receipts with a cutting torch.
...
| | |
|---|---|
| **Age / proportions** | 28 — adult, 7 heads |
| **Role** | ex-Akatomi test pilot, Iron Requiem field lead |
| **Memory point** | The old accident burn scar running from her right jaw down under the collar, deliberately left uncovered - and the company ID badge she still wears, punched through with a bolt. |
| **Secondary hook** | A pilot's harness worn over civilian clothes at all times, as if she might have to strap in within the minute. |
| **Silhouette class** | heavy shouldered wedge |
| **Shape language** | circle 5% / square 50% / triangle 45% |

**Silhouette masses** (Big:Mid:Small ≈ 6:3:1)

- **Big** — an armoured pilot jacket with hard shoulder yokes over a tank top - a broad, blunt block
- **Mid** — the harness straps crossing the torso; a short rough undercut with a long fringe
- **Small** — buckles, the punched badge, a lighter, knuckle tape
- **Negative space to protect** — the loops of the loose harness straps against the body
- **Black-fill read** — the squared shoulder yokes are the widest hard line in the roster

**Palette — 60-30-10.** Accent is reserved for the memory point and the eyes.

| slot | colour | hex |
|---|---|---|
| 60% dominant | scorched leather brown | `#3A2B22` |
| 30% secondary | olive service green | `#4E5C4A` |
| 10% accent | rust warning red | `#B4471F` |
| skin | | `#E0B594` |
| eyes | | `#B4471F` |
| shadow tint | | `#231A14` |

> shares Iron Requiem colours with Ren but inverted - Reika is dark-dominant where Ren is mid-value, so they never read the same in silhouette or thumbnail

**Symbol set** — copy literally into any prompt; paraphrasing starts drift.

- **Eyes** — narrow, hard, outer corner sharply up; small iris, minimal highlight - the least glossy eyes in the cast
- **Brows** — heavy, low, one broken by a small old scar
- **Mouth** — wide, thin-lipped, habitual flat line
- **Face shape** — strong adult jaw, visible tendon in the neck
- **Hair** — black with a grown-out bleach at the tips, undercut on the right, long swept fringe over the left eye
- **Build** — tall, heavily built for a woman of the cast, real functional muscle, adult hourglass under the jacket

**Wardrobe**

- default — Armoured pilot jacket with shoulder yokes over a dark tank top, five-point harness, reinforced cargo trousers, heavy boots, taped knuckles
- alt — Cockpit: full flight suit with the jacket left hanging on the seat
- props — punched Akatomi ID badge, portable cutting torch, battered flight helmet under the arm
...
- scar removed or minimised
- pin-up posing that contradicts her guardedness
- cute expression as default
...
saved_to: characters/reika/prompts/NN_turnaround.txt
```
...
**Pose.** feet planted wide, arms crossed high, head slightly tipped back - occupying space on purpose
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/reika/prompts/NN_sprite.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/reika/prompts/NN_expressions.txt
```
...
**Pose.** back-lit in a dry dock with the cutting torch lit, sparks arcing past her, the Scrap-Titan's foot behind her
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/reika/prompts/NN_hero.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/reika/prompts/NN_chibi.txt
```
...
python3 tools/triangulate_matte.py \
  characters/_wip/reika_white.png \
  characters/_wip/reika_black.png \
  characters/reika/reika_default_neutral.png \
  --alpha-out characters/_wip/reika_alpha.png

python3 tools/check_matte.py characters/reika/reika_default_neutral.png --report
python3 tools/check_matte.py characters/reika/reika_default_neutral.png --checks --out /tmp/reika_check.png
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/reika/prompts/NN_matte.txt
```
