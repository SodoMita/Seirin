...
`id: ryuki` · mountain · SR · density 7/10 · appeal track **cool_kid**

> Wears a dragon so she can be looked at on her own terms.
...
| | |
|---|---|
| **Age / proportions** | 16 — age-accurate teenage proportions, 6.5 heads |
| **Role** | Yuki's student, public speaker for the mountain village; never a fighter |
| **Memory point** | A lion-dance-derived dragon head worn as a hood, pushed back so it sits behind her like a second face looking up. |
| **Secondary hook** | Her ichthyosis is rendered as fine pale plating along the forearms and jawline - treated as texture and dignity, never as damage or horror. |
| **Silhouette class** | double-head hooded |
| **Shape language** | circle 40% / square 20% / triangle 40% |

**Silhouette masses** (Big:Mid:Small ≈ 6:3:1)

- **Big** — the pushed-back dragon hood creating a distinct second mass above and behind the skull
- **Mid** — a long segmented fabric spine trailing to calf height
- **Small** — scale-stitch rows, bell tassels, wrist wraps
- **Negative space to protect** — the gap between her real head and the dragon head is mandatory - they must not merge
- **Black-fill read** — two heads, one small one large - unique in the cast

**Palette — 60-30-10.** Accent is reserved for the memory point and the eyes.

| slot | colour | hex |
|---|---|---|
| 60% dominant | river jade costume | `#2E6B4F` |
| 30% secondary | bone cream underlayer | `#E9DFC7` |
| 10% accent | festival red tassel | `#D64545` |
| skin | | `#F0D3BA` |
| eyes | | `#C9A227` |
| shadow tint | | `#1C3D2E` |

> the pale plating is rendered as a slightly cooler, slightly higher-value skin tone with soft edges - never scabbed, never scaled like a reptile

**Symbol set** — copy literally into any prompt; paraphrasing starts drift.

- **Eyes** — large, slightly downturned outer corner - shy; strong highlight when she speaks
- **Brows** — soft, often inner-up
- **Mouth** — small, hidden behind a raised hand by default
- **Face shape** — soft youthful oval
- **Hair** — short dark green-black, uneven self-cut, one longer strand behind the ear
- **Build** — small, slight, 16

**Wardrobe**

- default — Jade dragon-dance overcoat with a segmented trailing spine, cream underlayer, wrapped forearms, straw-soled boots
- alt — Village daily: plain hoodie with the sleeves pulled over her hands
- props — dragon hood, a hand-written speech on folded paper, a bell tassel
...
- ichthyosis rendered as wounds, burns, gore, or monster scales
- combat use of the costume
- any sexualised framing
- pity framing
...
saved_to: characters/ryuki/prompts/NN_turnaround.txt
```
...
**Pose.** half-turned away, one hand at her mouth, dragon head looming gently behind
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/ryuki/prompts/NN_sprite.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/ryuki/prompts/NN_expressions.txt
```
...
**Pose.** at a village microphone, the dragon hood pulled fully up mid-motion, tassels flying, eyes visible and steady inside the jaw
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/ryuki/prompts/NN_hero.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/ryuki/prompts/NN_chibi.txt
```
...
python3 tools/triangulate_matte.py \
  characters/_wip/ryuki_white.png \
  characters/_wip/ryuki_black.png \
  characters/ryuki/ryuki_default_neutral.png \
  --alpha-out characters/_wip/ryuki_alpha.png

python3 tools/check_matte.py characters/ryuki/ryuki_default_neutral.png --report
python3 tools/check_matte.py characters/ryuki/ryuki_default_neutral.png --checks --out /tmp/ryuki_check.png
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/ryuki/prompts/NN_matte.txt
```
