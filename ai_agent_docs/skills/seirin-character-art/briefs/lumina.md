# Reina «Lumina» Kagami — art brief

`id: lumina` · chorus_abyss · SSR · density 10/10 · appeal track **hero_glamour**

> She does not believe in the miracle. She believes you need one.
...
| | |
|---|---|
| **Age / proportions** | 35 — adult, 7 heads |
| **Role** | leader of the Chorus of the Abyss; former crisis-communications specialist |
| **Memory point** | A high halo-collar of thin gold rods rising behind her head - a practical lighting rig that guarantees she is always the best-lit person in the room. |
| **Secondary hook** | Immaculate white gloves with a single fingerprint smudge of stage makeup on the right thumb. |
| **Silhouette class** | haloed column |
| **Shape language** | circle 10% / square 45% / triangle 45% |

**Silhouette masses** (Big:Mid:Small ≈ 6:3:1)

- **Big** — a floor-length column robe with a severe straight hem - no flare at all
- **Mid** — the radiating halo-collar; long sleeves that fall past the fingertips
- **Small** — rod tips, a slim throat mic, a row of tiny fastenings
- **Negative space to protect** — the gaps between the halo rods; the silhouette must read as a rayed disc behind a narrow column
- **Black-fill read** — column plus halo - reads as a religious icon at any size, which is exactly the manipulation

**Palette — 60-30-10.** Accent is reserved for the memory point and the eyes.

| slot | colour | hex |
|---|---|---|
| 60% dominant | abyss near-black | `#0E0A1A` |
| 30% secondary | ritual pale gold | `#E8D8A0` |
| 10% accent | chorus violet | `#6B3FA0` |
| skin | | `#F3D9C4` |
| eyes | | `#6B3FA0` |
| shadow tint | | `#070512` |

> always lit from below by a practical source at floor level; her key light is a prop and should be visible in wide shots

**Symbol set** — copy literally into any prompt; paraphrasing starts drift.

- **Eyes** — long, calm, warm-looking on purpose; the highlight is perfectly symmetrical in both eyes, which is subtly inhuman
- **Brows** — high, thin, serene arch
- **Mouth** — small, gentle, a professionally maintained smile
- **Face shape** — sculpted adult oval, high cheekbones
- **Hair** — silver-ash, centre-parted, waist-length and perfectly straight
- **Build** — tall, statuesque adult; the robe is completely concealing, and the allure is entirely staging, light and authority

**Wardrobe**

- default — Floor-length near-black column robe with pale gold ritual embroidery at the hem, gold halo-collar rig, long white gloves, throat mic
- alt — Off-stage: a plain charcoal business suit, halo packed in a flight case at her feet
- props — gold halo-collar lighting rig, the Tears of Kamikura demonstration vessel, a tablet of donor records
...
- real magic
- genuine supernatural effect
- cackling villain affect
- nudity or sexualised framing
...
saved_to: characters/lumina/prompts/NN_turnaround.txt
```
...
**Pose.** hands folded at the waist, chin level, halo softly lit, standing dead centre of her own frame
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/lumina/prompts/NN_sprite.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/lumina/prompts/NN_expressions.txt
```
...
**Pose.** on the ceremony platform with the halo at full output and the Tears of Kamikura gel weeping light, the crowd's phone screens below her
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/lumina/prompts/NN_hero.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/lumina/prompts/NN_chibi.txt
```
...
python3 tools/triangulate_matte.py \
  characters/_wip/lumina_white.png \
  characters/_wip/lumina_black.png \
  characters/lumina/lumina_default_neutral.png \
  --alpha-out characters/_wip/lumina_alpha.png

python3 tools/check_matte.py characters/lumina/lumina_default_neutral.png --report
python3 tools/check_matte.py characters/lumina/lumina_default_neutral.png --checks --out /tmp/lumina_check.png
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/lumina/prompts/NN_matte.txt
```
