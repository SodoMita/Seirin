# Aster-7 (ASTAR-7) — art brief

`id: aster7` · aquaforge · SR · density 5/10 · appeal track **mascot**

> A filing cabinet that would like to be your friend, and is keeping notes on how it is going.
...
| | |
|---|---|
| **Age / proportions** | non-human |
| **Role** | social robot prototype; log analysis, testimony recording, the ethical comic relief |
| **Memory point** | A face that is a single soft-cornered screen displaying two simple vector eyes and a mouth - and a visible thin bezel crack in the lower right corner that has never been repaired. |
| **Secondary hook** | One arm is a factory-standard white manipulator; the other is an obvious mismatched grey replacement. |
| **Silhouette class** | rounded appliance |
| **Shape language** | circle 50% / square 45% / triangle 5% |

**Silhouette masses** (Big:Mid:Small ≈ 6:3:1)

- **Big** — a capsule torso on a single wheeled column base - no legs
- **Mid** — the screen head on a short flexible neck; two thin arms
- **Small** — port covers, a status LED ring, a sticker someone put on his shoulder
- **Negative space to protect** — clear gap under the torso above the wheel base
- **Black-fill read** — capsule-on-wheel with a tilted rectangle head

**Palette — 60-30-10.** Accent is reserved for the memory point and the eyes.

| slot | colour | hex |
|---|---|---|
| 60% dominant | appliance white | `#EDF0F2` |
| 30% secondary | grey replacement parts | `#8E9BA6` |
| 10% accent | screen teal glow | `#7FD4CE` |
| skin | | `n/a` |
| eyes | | `#7FD4CE` |
| shadow tint | | `#4A5A66` |

> the screen is the only emissive element; it should be the brightest thing in any frame he appears in

**Symbol set** — copy literally into any prompt; paraphrasing starts drift.

- **Eyes** — two rounded vector rectangles on the screen; shape carries all emotion - no pupils
- **Brows** — drawn as two short screen strokes when needed
- **Mouth** — a single vector line: flat, curved, or a small open rounded rectangle
- **Face shape** — soft-cornered rectangle screen in a white bezel
- **Hair** — none
- **Build** — 1.1m tall capsule on a wheeled column

**Wardrobe**

- default — bare chassis with visible panel seams and one mismatched arm
- alt — field kit: a strapped-on sample case and a hi-vis band
- props — sample case, timestamped telemetry printout, a sticker of unknown origin
...
saved_to: characters/aster7/prompts/NN_turnaround.txt
```
...
**Pose.** slight forward lean, head tilted five degrees, both manipulators held together at the front
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/aster7/prompts/NN_sprite.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/aster7/prompts/NN_expressions.txt
```
...
**Pose.** in a dark server room, the screen face the only light source, projecting a timestamped evidence chain onto the wall
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/aster7/prompts/NN_hero.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/aster7/prompts/NN_chibi.txt
```
...
python3 tools/triangulate_matte.py \
  characters/_wip/aster7_white.png \
  characters/_wip/aster7_black.png \
  characters/aster7/aster7_default_neutral.png \
  --alpha-out characters/_wip/aster7_alpha.png

python3 tools/check_matte.py characters/aster7/aster7_default_neutral.png --report
python3 tools/check_matte.py characters/aster7/aster7_default_neutral.png --checks --out /tmp/aster7_check.png
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/aster7/prompts/NN_matte.txt
```
