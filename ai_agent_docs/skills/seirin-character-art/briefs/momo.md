# Momo Hoshizora — art brief

`id: momo` · akatomi · SSR · density 10/10 · appeal track **cool_kid**

> The city's manufactured hope, learning to read her own contract.
...
| | |
|---|---|
| **Age / proportions** | 15 — age-accurate teenage proportions, 6.5 heads |
| **Role** | Stella-5 lead vocalist, public voice of the team, works through adult editors and lawyers |
| **Memory point** | An in-ear monitor whose translucent cable is threaded with a fine constellation of fibre-optic star points that light in sequence when she sings. |
| **Secondary hook** | One glove on, one off - the bare hand is the one she uses to check the sound levels herself. |
| **Silhouette class** | wide star-burst |
| **Shape language** | circle 40% / square 15% / triangle 45% |

**Silhouette masses** (Big:Mid:Small ≈ 6:3:1)

- **Big** — layered asymmetric stage skirt, short at the front-right and long at the back-left - a swept comet shape
- **Mid** — twin high tails held by star clasps, one longer; a short structured shoulder cape
- **Small** — dense micro-detail - sequins, cable, mic clip, ear cuff, badge row
- **Negative space to protect** — the crescent gap under the swept skirt and between the cape and the arms
- **Black-fill read** — the comet-swept hem plus twin uneven tails - the only diagonal-hem silhouette in the cast

**Palette — 60-30-10.** Accent is reserved for the memory point and the eyes.

| slot | colour | hex |
|---|---|---|
| 60% dominant | stage white | `#F5F7FA` |
| 30% secondary | campaign blue | `#1F4FA8` |
| 10% accent | hot idol pink | `#FF5FA2` |
| skin | | `#FBDCC4` |
| eyes | | `#FF5FA2` |
| shadow tint | | `#3A4A80` |

> highest-key palette in the cast on purpose - she is the one lit for advertising. Her CG lighting must be visibly artificial.

**Symbol set** — copy literally into any prompt; paraphrasing starts drift.

- **Eyes** — very large, round, maximum gloss, star-shaped specular highlight (her only literal star)
- **Brows** — thin, high, mobile
- **Mouth** — wide, open smile is the default performance face
- **Face shape** — round, soft, idol-standard
- **Hair** — pale pink-blonde, twin high tails of uneven length, curled tips, blunt fringe with two long face-framing strands
- **Build** — petite, age-accurate 15, stage-costume coverage is full - long sleeves, high neck, opaque tights

**Wardrobe**

- default — White-and-blue structured stage jacket with a short cape, asymmetric layered skirt over opaque navy tights, tall white boots, single glove, in-ear monitor
- alt — Between takes: hoodie over the stage costume, one tail undone, holding a paper cup
- props — handheld stage mic (communication tool, never a weapon), fibre-optic in-ear monitor, annotated setlist
...
- revealing stage costume
- any sexualised framing, pose, or camera angle
- mic used as a weapon
- fan-service wardrobe malfunction
...
saved_to: characters/momo/prompts/NN_turnaround.txt
```
...
**Pose.** one hand on the in-ear monitor, other arm relaxed, weight on one hip, warm public smile
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/momo/prompts/NN_sprite.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/momo/prompts/NN_expressions.txt
```
...
**Pose.** mid-turn on stage under a hard key light, cape and hem flaring, one arm extended to the crowd, star highlight blown out
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/momo/prompts/NN_hero.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/momo/prompts/NN_chibi.txt
```
...
python3 tools/triangulate_matte.py \
  characters/_wip/momo_white.png \
  characters/_wip/momo_black.png \
  characters/momo/momo_default_neutral.png \
  --alpha-out characters/_wip/momo_alpha.png

python3 tools/check_matte.py characters/momo/momo_default_neutral.png --report
python3 tools/check_matte.py characters/momo/momo_default_neutral.png --checks --out /tmp/momo_check.png
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/momo/prompts/NN_matte.txt
```
