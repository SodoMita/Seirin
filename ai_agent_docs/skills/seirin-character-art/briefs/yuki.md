# Yuki Tenro — art brief

`id: yuki` · mountain · SSR · density 8/10 · appeal track **hero_glamour**

> Holds the line with paperwork and posture, and never draws the blade.
...
| | |
|---|---|
| **Age / proportions** | 19 — adult, 7 heads |
| **Role** | kendo instructor, archivist, non-violent negotiator for the mountain community |
| **Memory point** | A ceremonial shinai carried horizontally across her back in a document sling stuffed with rolled land-survey maps - the weapon is literally filed with the paperwork. |
| **Secondary hook** | Ink-stained right sleeve, pushed up and pinned with a bulldog clip. |
| **Silhouette class** | stable vertical rectangle with a horizontal bar |
| **Shape language** | circle 15% / square 60% / triangle 25% |

**Silhouette masses** (Big:Mid:Small ≈ 6:3:1)

- **Big** — long straight hakama-cut trousers and a fitted keikogi-derived jacket - an unbroken tall rectangle
- **Mid** — the horizontal shinai-and-map sling cutting across the back at shoulder-blade height
- **Small** — clip, cord ties, a small mountain-shrine charm
- **Negative space to protect** — the triangle between the sling and her back
- **Black-fill read** — the only cross-shaped silhouette in the roster - vertical body, horizontal sling

**Palette — 60-30-10.** Accent is reserved for the memory point and the eyes.

| slot | colour | hex |
|---|---|---|
| 60% dominant | cedar green gi | `#24402E` |
| 30% secondary | fog white trim | `#E3E9E6` |
| 10% accent | shrine vermilion cord | `#B4471F` |
| skin | | `#EEC9A8` |
| eyes | | `#4A3728` |
| shadow tint | | `#16281C` |

> the only vermilion in the cast belongs to her cord - it is her signature ten percent

**Symbol set** — copy literally into any prompt; paraphrasing starts drift.

- **Eyes** — narrow, long, level - calm and unimpressed; single hard highlight
- **Brows** — straight, level, medium weight
- **Mouth** — small, firm, corners naturally slightly down
- **Face shape** — clean oval with a defined jawline
- **Hair** — high tight ponytail, straight, blunt-cut ends; short curtain fringe parted centre
- **Build** — tall, athletic, genuine kendo shoulders and forearms, long neck

**Wardrobe**

- default — Indigo-green training jacket over a white underlayer, wide dark trousers, tabi-style boots, document sling with rolled maps and the sheathed shinai
- alt — Hearing day: dark blazer over the same white underlayer, maps in a hard case
- props — ceremonial shinai in a plain sheath, rolled survey maps, bulldog clip, hand-drawn watershed diagram
...
saved_to: characters/yuki/prompts/NN_turnaround.txt
```
...
**Pose.** feet shoulder-width, perfectly upright, hands resting one over the other at the front - formal readiness
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/yuki/prompts/NN_sprite.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/yuki/prompts/NN_expressions.txt
```
...
**Pose.** half-turned at the top of wet stone steps in fog, one hand steadying the sling, wind lifting the ponytail and map edges
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/yuki/prompts/NN_hero.txt
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/yuki/prompts/NN_chibi.txt
```
...
python3 tools/triangulate_matte.py \
  characters/_wip/yuki_white.png \
  characters/_wip/yuki_black.png \
  characters/yuki/yuki_default_neutral.png \
  --alpha-out characters/_wip/yuki_alpha.png

python3 tools/check_matte.py characters/yuki/yuki_default_neutral.png --report
python3 tools/check_matte.py characters/yuki/yuki_default_neutral.png --checks --out /tmp/yuki_check.png
```
...
negative_or_exclude: |
  <TO BE WRITTEN — every 'Never' item above + the global forbidden list>
saved_to: characters/yuki/prompts/NN_matte.txt
```
